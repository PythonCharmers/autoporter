# This code searches the Pypi repositories to the sets of python2 and python3 packages
...
def py2_classifiers():
    ...
    try:
        ...
        response.close()
    classifiers = data.decode('utf-8').splitlines()
    base_classifier = 'Programming Language :: Python'
    return (classifier for classifier in classifiers
            if classifier.startswith(base_classifier))
...
# Grab and pickle a set of all python packages

py2lists = py2_classifiers()

#py3lists = py3_classifiers()

tmp=[]

for i in py2lists:
    tmp.append(projects_matching_classifier(i))
       
    #for j in tmp:
    # print j

with  open('python2','w') as f:
    pickle.dump(tmp,f)

print "finished pickling all python packages"
#print py2set

# Grab and pickle a set of python 3 packages
...
with open('python3','w') as f:
    pickle.dump(tmp,f)
print "finished pickling python 3"
